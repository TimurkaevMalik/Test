...
/// Используется для описания имени файла и его расширения в типобезопасной
/// форме, чтобы повысить читаемость при загрузке ресурсов.
struct ResourceFile {
    let name: Name
...
extension ResourceFile {
    ...
    /// Предназначено для контроля форматов, доступных для загрузки (например,
    /// .plist, .json и т.д.)
